...
class Absensi extends CI_Controller
{
    ...
    public function simpan_status()
    {
        ...
        $simpan = $this->absensi->insert_data($data);
    ...
    public function absen()
    {
        ...
        $keterangan = $this->input->post('inpket');
        if ($keterangan == 'masuk') {
            $img = $this->input->post('inpgambar1');
            if ($img == '') {
                $this->session->set_flashdata('response', [
                    'status' => 'error',
                    'message' => 'Ambil Foto Dulu'
                ]);
        ...
        if ($keterangan == 'pulang') {
            $img = $this->input->post('inpgambar2');
            if ($img == '') {
                $this->session->set_flashdata('response', [
                    'status' => 'error',
                    'message' => 'Ambil Foto Dulu'
        ...
        $fileName = 'absen_'. strtolower($keterangan) . '_'. time() . '.png';
    ...
    public function export_pdf()
    {
        ...
        $this->pdf->load_view('absensi/print_pdf', $data);
    }
    ...
    //     $excel->getProperties()
    //             ->setCreator('PT Tiga Mitra Surabaya')
    //             ->setLastModifiedBy('PT Tiga Mitra Surabaya')
...
// End of File: d:\Ampps\www\project\absen_karyawan\application\controllers\Absensi.php 